...
/* Base del contributo: l'IMPONIBILE in fattura, IVA esclusa. Il DD 4/08/2026 art. 3 c. 5
   lett. a esclude «le spese per imposte, tasse e oneri di qualsiasi genere», e l'IVA è
   un'imposta: resta tutta a carico di chi paga. L'aliquota serve solo a mostrare il
   totale della fattura; 22% è l'ordinaria, modificabile in fondo alla pagina.
   Corretto il 26/09/2026 su segnalazione di uno spettatore: prima la pagina chiedeva
   la spesa IVA compresa e sovrastimava il contributo. */
var IVA=22;
...
var PASSI=[
 ...
 {id:"spesa", tipo:"numero", campo:"spesa", unita:"€, IVA esclusa", min:1, saltaSe:fuoriSubito,
  titolo: function(){ return fatto() ? "Quanto è l'imponibile della fattura, acquisto e posa?" : "Quanto pensi di spendere, acquisto e posa, IVA esclusa?"; },
  aiuto:"Scrivi l'<strong>imponibile</strong>, cioè la cifra <strong>senza IVA</strong>: l'80% si calcola su quella. Il decreto esclude «imposte, tasse e oneri di qualsiasi genere», e l'IVA è un'imposta, quindi la paghi tu per intero. Se hai solo il totale, dividilo per 1,22. Il contributo copre acquisto e <strong>posa in opera</strong>, comprese le opere elettriche ed edili strettamente necessarie, la progettazione, il collaudo e l'eventuale <strong>nuovo contatore</strong>."}
];
...
function disegnaPasso(){
  ...
  if(p.tipo==="numero"){
    ...
    var convN=function(){
      ...
      if(p.id==="spesa") c.textContent="Con l'IVA al "+num(S.iva)+"% la fattura fa "+eur(S.spesa*(1+S.iva/100))+". L'"+num(S.perc)+"% dell'imponibile fa "+eur(S.spesa*S.perc/100)+(S.spesa*S.perc/100>tetto()?", ma il tetto è "+eur(tetto())+": il contributo si ferma lì.":". Sotto il tetto di "+eur(tetto())+".");
...
/* ==================== IL CONTO ==================== */
function conto(){
  var spesa=Math.max(0,S.spesa||0);          /* imponibile, IVA esclusa */
  var iva=spesa*Math.max(0,S.iva||0)/100, totale=spesa+iva;
  var lordo=spesa*S.perc/100, contributo=Math.min(lordo,tetto());
  var soglia=tetto()/(S.perc/100);           /* imponibile oltre il quale il tetto morde */
  /* la detrazione di confronto, per un privato, si calcola sul pagato IVA compresa */
  var detr=totale*aliqDetr()/100, rata=detr/S.rate;
  return {spesa:spesa,iva:iva,totale:totale,lordo:lordo,contributo:contributo,tagliato:lordo>contributo,soglia:soglia,
          sogliaIvato:soglia*(1+Math.max(0,S.iva||0)/100),
          resta:totale-contributo,detr:detr,rata:rata,aTesta: condo()&&S.partecipanti>0 ? contributo/S.partecipanti : null,
          spesaTesta: condo()&&S.partecipanti>0 ? totale/S.partecipanti : null};
}
...
function bloccoSopra(){
  ...
  h+='<div class="carta" style="margin-top:2rem"><p class="occhiello">Il conto</p><h2>'+eur(K.contributo)+(condo()&&K.aTesta!==null?' al condominio, '+eur(K.aTesta)+' a famiglia':' di contributo')+'</h2>'+
    ...
    '<tr><td>Imponibile in fattura, acquisto e posa</td><td class="val">'+eur(K.spesa)+'</td></tr>'+
    '<tr><td>IVA al '+num(S.iva)+'% — fuori dal contributo</td><td class="val">'+eur(K.iva)+'</td></tr>'+
    '<tr><td>Totale della fattura</td><td class="val">'+eur(K.totale)+'</td></tr>'+
    '<tr><td>L\''+num(S.perc)+'% dell\'imponibile</td><td class="val">'+eur(K.lordo)+'</td></tr>'+
    ...
    '</table></div>';
  h+='<p>'+(K.tagliato
    ? '<strong>Il tetto morde.</strong> Sopra i '+eur(K.soglia)+' di imponibile ('+eur(K.sogliaIvato)+' IVA compresa) l\''+num(S.perc)+'% supera il massimale e il contributo si ferma a '+eur(tetto())+': ogni euro in più lo paghi tu per intero.'
    : '<strong>Sei sotto il tetto</strong>: fino a '+eur(K.soglia)+' di imponibile ('+eur(K.sogliaIvato)+' IVA compresa) il contributo resta l\''+num(S.perc)+'% pieno.')+' <span class="nota">L\'IVA non è mai coperta: il decreto esclude le imposte di qualsiasi genere (DD 4/08/2026 art. 3 c. 5 lett. a). Per questo, sul totale che paghi, il contributo non arriva mai all\''+num(S.perc)+'%.</span></p>';
...
function disegnaEsito(soloRicalcolo){
  ...
  if(!fuoriSubito()){
    ...
    hIp+='<div class="carta" style="margin-top:1.25rem"><p class="occhiello">Con che numeri ho fatto il conto</p><h2>Tutti modificabili, tutti con la fonte</h2><div class="griglia-ip">'+
      ipCampo("perc","Percentuale del contributo (%)",S.perc,1,"DPCM 10/06/2026 art. 6 c. 1: 80% dell'imponibile")+
      ipCampo("iva","Aliquota IVA in fattura (%)",S.iva,1,"22% ordinaria. L'IVA è esclusa dal contributo: DD 4/08/2026 art. 3 c. 5 lett. a")+
  ...
  [["perc","perc"],["iva","iva"],["tettopriv","tettoPriv"],["tettocondo","tettoCondo"],["detr","detr"],["rate","rate"],["ticket","ticket"],["dotazione","dotazione"]].forEach(function(c){
...
function costruisciReport(){
  ...
  h+='<h2>Il mio caso, in breve</h2><table><tr><th>Voce</th><th>Risposta</th></tr>'+
    ...
    riga("Imponibile dichiarato (IVA esclusa)", eur(K.spesa)+" — totale fattura con IVA al "+num(S.iva)+"%: "+eur(K.totale))+
